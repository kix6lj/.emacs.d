...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    (c-mode-hook . (eglot-ensure t))
    (c++-mode-hook . (eglot-ensure t))
  
    :config
    (setq eldoc-echo-area-use-multiline-p nil))
#+end_src
*** How to get Eglot work with remote python developement through TRAMP
Since most of my code are developed and run on a remote server. I need figure out how to use Eglot for remote dev. This is especially difficult for python since I use anaconda for environment management.
So I come up with the following ~.dir-locals.el~ file template for each project.
